...
@Api(value = "/weatherAuth", authorizations = {@Authorization(value = "bearerAuth")})
@SwaggerDefinition(
...
public class WeatherResourceAuth {
    ...
    public WeatherResourceAuth(List<WeatherData> weatherData) {
    ...
    @ApiOperation(value = "Aktuálne počasie", notes = "Získanie dát o aktuálnom počasí. Dáta sú obnovované každú " +
            "minútu.")
    @Timed
    @RolesAllowed({"current","all"})
    public Optional<WeatherData> getCurrentWeather(@ApiParam(value = "ID meteo stanice", required = true, example =
            "station_1") @PathParam(
